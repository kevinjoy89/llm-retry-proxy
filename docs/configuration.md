...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `SSE2WS_MODE` | `off` | `off` = 握手返回 426 并由客户端回退 HTTP；`bridge` = 接收 Responses WebSocket 并代理到上游 |
| `SSE2WS_UPSTREAM_TRANSPORT` | `auto` | `auto` = 优先原生 WebSocket，明确不支持时回退 HTTP/SSE；`websocket` = 仅原生 WebSocket；`sse` = 始终桥接到 HTTP/SSE |
...
该模式只接受匹配到 Responses API 的 WebSocket 路径。`auto` 会先向相同上游路径发起 WebSocket Upgrade；上游以 200、404、405、426 或 501 明确拒绝 Upgrade 时，才切换到 SSE→WebSocket 桥接。鉴权失败、限流、服务异常和网络故障不会误触发回退。原生 WebSocket 模式双向透传文本和二进制帧，SSE 桥接模式只接受文本 JSON `response.create`，同一连接一次只允许一个生成请求。SSE 桥接中的 `generate=false` warmup 由代理本地完成，不产生上游请求或统计记录。
