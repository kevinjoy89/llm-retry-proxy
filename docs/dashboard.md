...
- **总览**：总请求数、总重试次数、上游可用率、下游可用率、失败请求数、P95 耗时、Token 用量和缓存命中
- **号池健康**：累计可用率与当前状态分开统计；冷却期标记为“熔断”，冷却结束但最近一次有效检测仍失败的 key 标记为“不可用”，SSE2WS 首事件超时等中性观测不会把更早的失败继续显示为当前不可用，最近无观测则标记为“待检测”
...
- **明细表**：按供应商、按模型展示 Token 与缓存数据，并提供按路径 Top 10
