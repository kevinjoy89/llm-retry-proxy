...
字段路径使用点分隔，例如 `data.items` 或 `metrics.ttft`；接口直接返回顶层数组时，记录列表路径填写 `$`。延迟单位可以选择毫秒或秒。URL 可以自带其它查询参数，代理不会把号池登录令牌或 Key 发送给该地址。当前数据源请求固定使用 GET，不支持自定义请求头或执行 JavaScript。
