...
- `SSE2WS_MODE=bridge` 时使用独立的首事件预算 `SSE2WS_FIRST_EVENT_TIMEOUT`。桥接与纯 SSE 共用响应头之前的 HTTP 重试、单 Key 响应头超时和换 Key 逻辑；最终上游返回 2xx 响应头后，才开始独立计算首事件超时。首事件超时会结束该轮并在 Key 可用率中记为中性结果，不在桥接层重连、不熔断 Key。空流、明确错误事件、非法 SSE 或传输失败仍会按流失败处理。
