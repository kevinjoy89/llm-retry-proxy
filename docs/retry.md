...
- `SSE2WS_MODE=bridge` 时使用独立的首事件预算 `SSE2WS_FIRST_EVENT_TIMEOUT`，同时覆盖响应头和第一个合法 SSE 事件。桥接层在任何事件发给客户端之前，可按 `SSE2WS_FIRST_EVENT_RETRIES` 使用同一 key 重连；耗尽后才熔断换 key。桥接请求不再叠加 `RESPONSES_ATTEMPT_HEADER_TIMEOUT`，避免旧的响应头预算提前切换 key。
